...
<body>
				...
				<div class="col-md-12">
					...
					<div class="p-2">
						...
						<div class="mt-2 text-right">
							...
							<button class="btn btn-sm ml-1 shadow-none maincolor-re1" type="button" id="cancelBtn">취소</button>
						</div>
					</div>
				</div>
			</div>
	...
	function selectReplyList(){
		...
		$.ajax({
			url : "${contextPath}/cafeReply/selectReplyList/" + cafeNo,
			type : "post",
			dataType : "json",
	...
	$("#addReply").on("click", function(){
		...
		if(memNo == 0){
			...
			if(replyContent.trim().length == 0){
				...
				$.ajax({
					url : "${contextPath}/cafeReply/insertReplyList/" + cafeNo,
					type : "post",
	...
	function updateReply(replyNo, el){
		...
		if(replyContent.trim().length == 0){
			...
			$.ajax({
				url : "${contextPath}/cafeReply/updateReply/" + replyNo,
				type : "post",
	...
	function deleteReply(replyNo){
		...
		if(confirm("정말로 삭제하시겠습니까?")){
			...
			$.ajax({
				url : "${contextPath}/cafeReply/deleteReply/" + replyNo,
				success : function(result){
	...
	function addChildReply(el, parentReplyNo){
		...
		if(replyContent.trim().length == 0){ // 대댓글 미작성 시
			...
			$.ajax({
				url : "${contextPath}/cafeReply/insertChildReply/" + cafeNo,
